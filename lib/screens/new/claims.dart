...
class ClaimsScreen extends StatefulWidget {
  ...
  const ClaimsScreen({Key? key, this.refreshToken = 0}) : super(key: key);
...
class _ClaimsScreenState extends State<ClaimsScreen> {
  ...
  @override
  void didUpdateWidget(covariant ClaimsScreen oldWidget) {
  ...
  Future<void> _loadClaims() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _liveClaims = claims.map(_mapBackendClaim).toList();
        _isLoading = false;
      });
    } on ServerException {
      if (!mounted) return;
  ...
  Map<String, dynamic> _mapBackendClaim(Map<String, dynamic> claim) {
    final statusRaw = (claim['status'] ?? '').toString().toLowerCase();
    final status = _statusLabel(statusRaw);
    final rawAmount =
        claim['amount'] ?? claim['estimated_payout'] ?? claim['expectedPayout'];
    ...
    return {
      ...
      'date': _formatDate(
        claim['event_date']?.toString() ??
            claim['date']?.toString() ??
            claim['event_timestamp']?.toString(),
      ),
      'amount': amount.round(),
      'status': status,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await Navigator.pushNamed(context, AppRoutes.apply);
          if (!mounted) return;
          _loadClaims();
        },

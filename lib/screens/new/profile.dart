...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final historyRaw = ((_workerData?['recent_history'] as List<dynamic>?) ??
    ...
    final memberSince = (_workerData?['registered_at'] ??
            _workerData?['member_since'] ??
            'N/A')
        .toString();
    final totalProtected =
        (stats['total_protected_amount'] ?? _workerData?['total_protected_amount'] ?? 0)
            .toString();
    ...
    final locationAddress = (currentLocation['address'] ?? 'Unknown').toString();
    ...
    final history = historyRaw
        .map(
          (h) => {
            'incident': (h['incident'] ?? h['event_type'] ?? 'Claim').toString(),
            'date': (h['date'] ?? h['submitted_at'] ?? 'Recent').toString(),
            'detail':
                (h['detail'] ?? h['status'] ?? 'Claim activity recorded')
                    .toString(),
          },
        )
        .toList();
